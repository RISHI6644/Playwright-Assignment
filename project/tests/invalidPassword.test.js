...
test('Invalid password', async ({ page }) => {
    ...
    logger.info('Starting inValidPassword test');
    ...
    // Generate and fill password field with a randomly generated invalid password
    const randomInvalidPassword = await formSteps.generateRandomInvalidPassword();
    await formSteps.mainPage.passwordField.fill(randomInvalidPassword);
    logger.info(`Filled password field with: ${randomInvalidPassword}`);
    ...
    // Verify that the next card is not open by checking the next card button is not visible
    const isNextCardVisible = await formSteps.mainPage.isNextCardButtonVisible();
    expect(isNextCardVisible).toBeFalsy();
    logger.info('Verified next card is not open');
});
